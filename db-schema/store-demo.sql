...
CREATE VIEW `color_chips` AS
(
select `i`.`prodID`                                                    AS `prodID`,
       ...
       group_concat((select group_concat(coalesce(`ic1`.`pclrpID`, 0) separator ',')
                     from `inventory_colors` `ic1`
                     where (`ic1`.`piID` = `i`.`piID`)) separator '|') AS `color_photos`,
       group_concat(`ic`.`have_chip` separator '|')                    AS `have_chips`,
       group_concat(`ic`.`pclrID` separator '|')                       AS `color_ids`,
       (select group_concat(`pp`.`ppID` separator '|')
        from `product_photos` `pp`
        where (`pp`.`prodID` = `i`.`prodID`)
        order by `pp`.`position`)                                      AS `product_photos`
from (`inventory` `i`
         left join `inventory_colors` `ic` on ((`ic`.`piID` = `i`.`piID`)))
group by `i`.`prodID`);
...
(
select `pi`.`piID`                                                                      AS `piID`,
       `pi`.`prodID`                                                                    AS `prodID`,
       `pi`.`stock_amount`                                                              AS `stock_amount`,
       `pi`.`insert_date`                                                               AS `insert_date`,
       `pi`.`price`                                                                     AS `price`,
       `pi`.`old_price`                                                                 AS `old_price`,
       `pi`.`buy_price`                                                                 AS `buy_price`,
       `pi`.`weight`                                                                    AS `weight`,
       `pi`.`size_value`                                                                AS `size_value`,
       `pi`.`pclrID`                                                                    AS `pclrID`,
       `pi`.`color`                                                                     AS `color`,
...
from ((((`product_inventory` `pi` left join `product_colors` `pclr` on ((`pclr`.`pclrID` = `pi`.`pclrID`))) left join `store_colors` `sc` on ((`sc`.`color` = `pclr`.`color`))) left join `products` `p` on ((`p`.`prodID` = `pi`.`prodID`)))
         left join `store_promos` `sp` on (((`sp`.`targetID` = `p`.`catID`) and (`sp`.`target` = 'Category') and
...
select `si`.`color`      AS `color`,
       ...
       `si`.`piID`       AS `piID`,
       `si`.`prodID`     AS `prodID`,
       `si`.`have_chip`  AS `have_chip`,
       `si`.`pclrpID`    AS `pclrpID`,
       `si`.`ppID`       AS `ppID`,
       `sc`.`color_code` AS `color_code`
from (`inventory` `si`
         join `store_colors` `sc` on ((`sc`.`color` = `si`.`color`)))
where (`si`.`pclrID` is not null)
group by `si`.`pclrID`);
...
(
select min(`si`.`sell_price`)                          AS `price_min`,
       max(`si`.`sell_price`)                          AS `price_max`,
       (select group_concat(`pcp`.`pclrpID` order by `pcp`.`position` ASC separator '|')
        from `product_color_photos` `pcp`
        where (`pcp`.`pclrID` = `si`.`pclrID`))        AS `color_gallery`,
       ...
       `cc`.`pi_ids`                                   AS `color_pids`,
       `cc`.`colors`                                   AS `colors`,
       `cc`.`color_photos`                             AS `color_photos`,
       `cc`.`have_chips`                               AS `have_chips`,
       `cc`.`color_ids`                                AS `color_ids`,
       `cc`.`product_photos`                           AS `product_photos`,
       `cc`.`color_codes`                              AS `color_codes`,
       `si`.`piID`                                     AS `piID`,
       `si`.`stock_amount`                             AS `stock_amount`,
       `si`.`price`                                    AS `i_price`,
       `si`.`old_price`                                AS `i_old_price`,
       `si`.`buy_price`                                AS `i_buy_price`,
       `si`.`weight`                                   AS `i_weight`,
       `si`.`size_value`                               AS `size_value`,
       `si`.`pclrID`                                   AS `pclrID`,
       `si`.`color`                                    AS `color`,
       ...
       `si`.`insert_date`                              AS `inventory_date`,
       `p`.`prodID`                                    AS `prodID`,
       `p`.`class_name`                                AS `class_name`,
       `p`.`brand_name`                                AS `brand_name`,
       `p`.`product_code`                              AS `product_code`,
       `p`.`product_name`                              AS `product_name`,
       `p`.`product_summary`                           AS `product_summary`,
       ...
       `p`.`view_counter`                              AS `view_counter`,
       `p`.`order_counter`                             AS `order_counter`,
       `p`.`visible`                                   AS `visible`,
       `p`.`promotion`                                 AS `promotion`,
       `p`.`importID`                                  AS `importID`,
       `p`.`update_date`                               AS `update_date`,
       `p`.`insert_date`                               AS `insert_date`
from ((`inventory` `si` join `products` `p` on ((`p`.`prodID` = `si`.`prodID`)))
         left join `color_chips` `cc` on ((`cc`.`prodID` = `si`.`prodID`)))
where (`p`.`visible` = 1)
